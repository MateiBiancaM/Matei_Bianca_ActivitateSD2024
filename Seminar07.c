...
//alg pt calcularea codului hash=>pt inserarea in vector=>ret poz
int calculHash(int cod, const char* nume, int dimensiune) {
	if (dimensiune > 0) {
		int rez = cod * strlen(nume);//tb sa ne incadram in dim
		return rez % dimensiune;
	}
	return -1;
}

void inserareFinal(nod** cap, Galerie galerie) {
...
//inserarea galerie in tabela
void inserareGalerieInTabela(HashTable hash, Galerie gal) {
	if (hash.dim > 0) {
		int pozitie = calculHash(gal.cod, gal.nume, hash.dim);//poz pe care realizez inserare
		//verif daca avem coleziune
		if (hash.vector[pozitie] != NULL) {
			inserareFinal(&(hash.vector[pozitie]), gal);
		}
		else {
			inserareFinal(&(hash.vector[pozitie]), gal);
		}
